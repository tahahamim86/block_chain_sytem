...
from .utils import rebuild_blockchain  # Import the function to rebuild the blockchain
...
def get_user_docs_by_token(request, app_user_id):
    ...
    if access.token != token:
        return Response({"status": "Access denied: Invalid token"}, status=status.HTTP_403_FORBIDDEN)
    ...
    # Rebuild the blockchain every time this API is accessed
    rebuild_blockchain()
